...
int main(int n_argc, char** ppch_argv) {
    ...
    try {
        ...
        case CARGoSCommandLineArgParser::ACTION_RUN_EXPERIMENT: {
            ...
            for (CSpace::TMapPerType::iterator it = cEntities.begin(); it != cEntities.end();
                 ++it) {
                CControllableEntity* pcEntity = any_cast<CControllableEntity*>(it->second);
                try {
                    CRVRNEATController& cController =
                        dynamic_cast<CRVRNEATController&>(pcEntity->GetController());
            ...
            RVRCoreLoopFunctions& cLoopFunctions =
                dynamic_cast<RVRCoreLoopFunctions&>(cSimulator.GetLoopFunctions());
