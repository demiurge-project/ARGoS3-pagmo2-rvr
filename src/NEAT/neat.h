...
namespace NEAT {
	...
	extern double mutate_add_sensor_prob; // NEW: probability to connect a sensor to all outputs (useful if we start with some sensors disconnected).
	...
	extern int budget; // Total number of times the objective function (usually argos run) is called


	/* New parameters added */
	extern unsigned int num_gens; //number of generations.
	extern unsigned int num_runs_per_gen; //number of times to run the experiment per generation.
	extern unsigned int num_runs_post_eval; //number of times to run the post evaluation of the population.
	extern double weight_lower_bound; //lower bound for the weights.
	extern double weight_upper_bound; //upper bound for the weights.
	extern double elitism_percentage; //percentage of elitism
	extern bool all_inputs_disconnected; //check if all the inputs are disconnected. It's checked in the code when we load a genome from a file.

	extern double step_size; // Step size parameter for cmaes algorithm
	extern bool use_cmaes; // Boolean to define if the cmaes algorithm should be used or xnes
