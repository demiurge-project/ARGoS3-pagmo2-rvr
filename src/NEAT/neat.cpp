...
/* New Parameters */
unsigned int NEAT::num_gens = 0;
unsigned int NEAT::num_runs_per_gen = 1;
unsigned int NEAT::num_runs_post_eval = 0;
double NEAT::weight_lower_bound = -8.0; // initial value that was in the original code in Genome::mutate_link_weights(...).
double NEAT::weight_upper_bound = 8.0; // initial value that was in the original code in Genome::mutate_link_weights(...).
double NEAT::elitism_percentage = 0.0;
bool NEAT::all_inputs_disconnected = true;

double NEAT::step_size = 0.0;
bool NEAT::use_cmaes = true;
...
bool NEAT::load_neat_params(const char *filename, bool output) {
   ...
   if(output)
      ...
      paramFile >> curword;
      paramFile >> NEAT::num_runs_post_eval;
    ...
    if(output) {
	    ...
	    //printf("mutate_add_sensor_prob=%f\n",weight_upper_bound);
	    ...
	    printf("num_runs=%d\n",num_runs);
	    printf("num_gens=%d\n",num_gens);
	    printf("num_runs_per_gen=%d\n",num_runs_per_gen);
	    printf("num_runs_post_eval=%d\n",num_runs_post_eval);
	    printf("weight_lower_bound=%f\n",weight_lower_bound);
	    printf("weight_upper_bound=%f\n",weight_upper_bound);
	    printf("elitism_percentage=%f\n",elitism_percentage);
    }
