...
Genome::Genome(Network* net) {
	nodes = net->all_nodes;

	/*
	std::cout << "New trait:" << std::endl;
	std::vector<NNode*>::iterator curnode;
	for(curnode=nodes.begin();curnode!=nodes.end();++curnode) {

		traits.push_back((*curnode)->nodetrait);
	}
	*/
	...
	std::vector<Link*>::iterator curlink;
	for(curlink=links.begin();curlink!=links.end();++curlink) {
		genes.push_back(new Gene(*curlink));
	}
	genome_id = net->net_id;
}
...
void Genome::print_to_file(std::ofstream &outFile) {
  ...
  for(curtrait=traits.begin();curtrait!=traits.end();++curtrait) {
    ...
    std::cout << "trait id set: " << std::endl;
    (*curtrait)->print_to_file(outFile);
    std::cout << "Print succeeded: " << std::endl;
  }
...
/*
	...
	*/
	...
	for(curgene=genes.begin();curgene!=genes.end();curgene++) {
		...
		//The following determines if the probabilities of doing cold gaussian
		...
		if (!((*curgene)->frozen)) {
			...
			else if ((gene_total>=10.0)&&(num>endpart)) { //num is set to 0.0
			...
			//Keep the weights between weight_lower_bound and weight_upper_bound (experimental)
			if (((*curgene)->lnk)->weight > NEAT::weight_upper_bound)
				((*curgene)->lnk)->weight = NEAT::weight_upper_bound;
			else if (((*curgene)->lnk)->weight < NEAT::weight_lower_bound)
				((*curgene)->lnk)->weight = NEAT::weight_lower_bound;
...
// Immediately connects a randomly chosen sensor which was previously disconnected to all the outputs at once.
// Useful if we start with some (inputs) sensors disconnected.
// NOTE: need to decomment the parameter in NEAT::load_neat_params in order to be taken into account.
